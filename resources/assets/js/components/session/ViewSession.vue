...
<script>
    export default {
        ...
        methods: {
            update(session){
                axios.patch('/session/'+ this.id, session)
                    .then(response => {
                        ...
                        if (_response.status === 0){
                            ...
                            console.log(_response.errors.name);
                            this.$notify({type: 'error', text: '<span style="color: white">'+ _response.errors.name[0] +'</span>', speed:400});
                        }
                    })
                    .catch(error =>{
